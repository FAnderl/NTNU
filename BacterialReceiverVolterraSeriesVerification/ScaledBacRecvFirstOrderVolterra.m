function H = BacRecvFirstOrderVolterra(order,n,v, q1,kappaApi, kApi,deltaRx,...
    c_tilde, k1,k2, delta_ccp, a_tilde,kP, k_Dp, delta_Api, betaE, deltaE,input_fct,...
...
H =@(s) ((k1*c_tilde*constAIP*(tc./constCcp)) ./ ((s + tc*(k2 + delta_ccp)))) .*...      % C_CP = C + AIP / P
...
if(bitget(order,2))
    ...
    H =@(s) H(s) .* ...                                          % A_PI = C_CP + A 
                ((kP*a_tilde*tc*(constCcp/constApi)) ./ (n*v*(s + tc*(k_Dp + delta_Api))));             
end
...
if(bitget(order,3))                                                         % A_Pi -> R_(X)
    ...
    H =@(s) H(s) .* ...                                         
                ((n*kappaApi*q1*(tc/constRx))./((s + tc*deltaRx)));
end
...
if(bitget(order,4))                                                         % R_(x) -> E_Lux
    ...
    H =@(s) H(s) .*...   
                  ((betaE*constRx*(tc/constElux)) ./ ((s+tc*deltaE)));
end
