function [dx,y] = RED_FullyIntegratedModel4GreyBox(t,x,u,  ...
    ...
    , kappaX, k_SigX,deltaSigX, Ve,v,...
      eta, RLUconst,deltaEL,alpha_L,  varargin)
...
dx0(1,1) = - (Df*1e6) *  bac_gro_at_t * (x0(1)/(bac_gro_at_t*v) - x0(6)) - delta_XIP_int*x0(1)...
        - 1/(bac_gro_at_t*v)  * k_TF_m * (x0(1) * x0(2))^dim_stoch_exp + k_TF_f* x0(3); 


% Free comR
dx0(2,1) =  bac_gro_at_t * alpha_comR * kappaB  + k_TF_f* x0(3)  - ...
                      1/(bac_gro_at_t*v)  * k_TF_m * (x0(1) * x0(2))^dim_stoch_exp - ...
                      delta_tf * x0(2)   + ...
...
dx0(3,1) =  1/2 * 1/(bac_gro_at_t*v)  * k_TF_m * (x0(1) * x0(2))^dim_stoch_exp - ...
...
dx0(4,1) =  bac_gro_at_t * alpha_L * kappa_comR *  (((x0(3))/(bac_gro_at_t*v))/ (k_comR+((x0(3))/(bac_gro_at_t*v)))) ...
        - deltaEL * x0(4); 
...
% mRNA - sigX
% dx0(7,1) =  bac_gro_at_t * kappa_comR *  (((x0(3))/(bac_gro_at_t*v))/ (k_comR+((x0(3))/(bac_gro_at_t*v)))) ...
%         - deltaR * x0(7); 
