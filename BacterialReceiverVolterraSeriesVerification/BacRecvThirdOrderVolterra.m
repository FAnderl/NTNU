function H = BacRecvThirdOrderVolterra(order, n,v, q3, kappaApi, kApi,deltaRx, c_tilde, k1,k2, delta_ccp, a_tilde,kP, k_Dp, delta_Api, betaE, deltaE,input_fct)
%UNTITLED First Order Volterra Transfer FUnction/ Frequency Response
%   see Bacterial Receiver
H =@(s1, s2, s3) -(k1/3)*( ( ( -(1/2)*k1.* ((((k1*c_tilde) ./ (s1 + k2 + delta_ccp)) + ((k1*c_tilde) ./ (s2 + k2 + delta_ccp))) ./...      % C_CP = C + AIP / P
                  (s1 +s2+k2 + delta_ccp)) ...
                ) +...                
                (-( s1/2)*k1.*((((k1*c_tilde) ./ (s1 + k2 + delta_ccp)) + ((k1*c_tilde) ./ (s3 + k2 + delta_ccp))) ./...     
                 (s1 +s3 + k2 + delta_ccp)) ...
                 ) +...
...
if(bitget(order,3))                                                         % A_Pi -> R_(X)
    ...
    H =@(s1, s2, s3) H(s1,s2, s3) .* ...                                         
  (q3*(n*kappaApi)/((s1+s2+s3 + deltaRx)));
end
